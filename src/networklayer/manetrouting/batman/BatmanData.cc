...
int8_t Batman::send_udp_packet(cPacket *packet_buff, int32_t packet_buff_len, const Uint128 & destAdd, int32_t send_sock, BatmanIf *batman_if)
{
    ...
    //add by Eugen
    if(destAdd == ((in_addr_t) 0xFFFFFFFF)){
        manetStats::getInstance()->incPacketBroadcastSend();
    }
    else{
        manetStats::getInstance()->incPacketUnicastSend();
    }
    manetStats::getInstance()->addToTotalSendProtokollBytes(packet_buff->getByteLength());
    //end add
