...
//add by Eugen
#ifdef RPS
 ChannelControl* cc = dynamic_cast<ChannelControl *>(ChannelAccess::getChannelControl());
double rcvdPower = cc->getRcvdPower(getRadioPosition(),framePos);
#else
...
void Radio::handleLowerMsgEnd(AirFrame * airframe)
{
    ...
    else
    {
        ...
        //add by Eugen
//        collStats::getInstance()->addCollPacket(airframe);
        IInterfaceTable *ift = InterfaceTableAccess().getIfExists();
        if(ift){
            for(int i = 0; i<ift->getNumInterfaces(); i++){
                InterfaceEntry* iEntry = ift->getInterface(i);
                if(!iEntry){
                    continue;
                }
                cPacket *tempFrame = airframe->getEncapsulatedPacket();
                if(dynamic_cast<Ieee80211DataFrame *>(tempFrame)){
                    Ieee80211DataFrame *tempIeee = dynamic_cast<Ieee80211DataFrame *>(tempFrame);
                    if(tempIeee->getReceiverAddress().isBroadcast() || iEntry->getMacAddress() == tempIeee->getReceiverAddress()){
                        collStats::getInstance()->addCollPacket(airframe);
                    }
                }
                else if(dynamic_cast<Ieee80211ACKFrame *>(tempFrame)){
                    Ieee80211ACKFrame *tempIeee = dynamic_cast<Ieee80211ACKFrame *>(tempFrame);
                    if(tempIeee->getReceiverAddress().isBroadcast() || iEntry->getMacAddress() == tempIeee->getReceiverAddress()){
                        collStats::getInstance()->addCollPacket(airframe);
                    }
                }
